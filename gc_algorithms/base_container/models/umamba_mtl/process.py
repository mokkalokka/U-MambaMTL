...
from UMambaBot_3d_mtl import UMambaBotMTL
import os
...
class csPCaAlgorithm(SegmentationAlgorithm):
    ...
    def __init__(self):
        ...
        self.monai_transforms = transforms.Compose(
            ...
            transforms.SpatialPadd(keys=self.all_keys, spatial_size=[128,128,20], mode="reflect"),
            ...
            transforms.CropForegroundd(keys=self.all_keys, 
                                            ...
                                            allow_smaller=False, 
                                            mode="reflect"),
            transforms.CenterSpatialCropd(keys=self.all_keys, roi_size=[256,256,20]),
    ...
    def predict(self):
        ...
        for p in range(len(self.models)):
            ...
            with torch.no_grad():
                ...
                logits = self.models[p](x["image"][None])
                pred = torch.sigmoid(logits)[:, 1, ...].detach().cpu().numpy()
                outputs += [pred[0]]
                
                # Predict prostate (via zones)
                ...
                prostate_probs.append(prostate_probability.detach().cpu().numpy()[0][0])
                
        # ensemble softmax predictions
        ensemble_output = np.mean(outputs, axis=0).astype('float32')
        ensemble_prostate_prob = np.mean(prostate_probs, axis=0).astype('float32')
        prostate_pred = (ensemble_prostate_prob > 0.5).astype('float32')
        print(f"ensemble_output shape: {ensemble_output.shape}")
        print(f"prostate_pred shape: {prostate_pred.shape}")
        
        # Filter out predictions outside prostate
        ensemble_output = ensemble_output * prostate_pred
