...
class nnUNetTrainer(object):
    ...
    def _build_loss(self):
        ...
        #                            'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp}, {}, weight_ce=1, weight_dice=1, # TODO changed 'do_bg': False !
        #                           ignore_label=self.label_manager.ignore_label, dice_class=MemoryEfficientSoftDiceLoss)
        
        loss = FocalLoss(apply_nonlin=nn.Softmax(dim=1), gamma=1)
        # loss = FocalLoss(alpha=[0.75, 0.25], apply_nonlin=nn.Softmax(dim=1), gamma=1)
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.AdamW(self.network.parameters(), self.initial_lr, weight_decay=1e-4)
        lr_scheduler = PolyLRScheduler(optimizer, self.initial_lr, self.num_epochs)
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True, dtype=torch.float16) if self.device.type == 'cuda' else dummy_context():
            output = self.network(data)
            # print(f"pred shape: {output[0][0,1,...].shape}")
            # max_idx = torch.argmax(output[0][0,1,...])
            # # print(max_idx)
            # # print(f"min_pred: {[output[0][0,i,...].min().item() for i in range(2)]}")
            # max_preds = [output[0][0,i,...].flatten()[max_idx].item() for i in range(2)]
            # print(f"max_pred: {max_preds}")

            # del data
            l = self.loss(output, target)
            
            # print(target[0].max().item())
    ...
    def on_train_epoch_end(self, train_outputs: List[dict]):
        ...
        if self.is_ddp:
            ...
            print(f"Loss min: {np.min(outputs['loss'])}")
            print(f"Loss max: {np.max(outputs['loss'])}")
